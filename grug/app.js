const STORAGE_KEY = "dnd_grug_character_v1";
...
function exportSave(){
  const payload = { app:"dnd-grug", version:2, exportedAt:new Date().toISOString(), data:state };
  const blob = new Blob([JSON.stringify(payload, null, 2)], { type:"application/json" });
  const url = URL.createObjectURL(blob);
  const link = document.createElement("a");
  link.href = url;
  link.download = `jefferson-grug-${new Date().toISOString().replace(/[:.]/g, "-")}.json`;
