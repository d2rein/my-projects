...
const DEFAULT_FEED = {
  ...
  sources: [{ name: "Official Pokemon GO news", url: "https://pokemongolive.com/" }],
  releases: []
};
...
function setup() {
  ...
  bindEvents();
  addLog("info", "Tracker opened", { activeList: state.activeList, scanMode: state.scanMode });
  renderAll();
  requestMissingNames();
  drawIdleCanvas();
}
...
function addLog(level, message, details = null) {
  state.logs.push({ at: new Date().toISOString(), level, message, details });
...
async function startCapture() {
  ...
  scrollHint.textContent = "Manual box alignment stays fixed. Pause briefly on each new row block for the scan.";
...
function drawOverlay(rect) {
  ...
  if (lastScanTiles.length) {
    ...
    previewCtx.font = `700 ${Math.max(16, Math.round(tileHeight * 0.12))}px Trebuchet MS`;
    lastScanTiles.filter(tile => tile.confirmed).forEach(tile => {
      const x = rect.x + tile.col * tileWidth + tileWidth - 34;
      const y = rect.y + tile.row * tileHeight + 24;
      previewCtx.fillText("OK", x, y);
    });
  }
  previewCtx.restore();
}
...
async function scanCurrentFrame(mode) {
  if (isWorkerBusy || !previewHasFrame) return;
  ...
  try {
    ...
    if (!results.length) {
      ...
      lastScanDetail.textContent = "Try pausing on a clearer frame or slightly widening the number band.";
      addLog("warn", "Scan found no dex numbers", { mode, list: state.activeList, motion });
      return;
    }
    ...
    lastScanDetail.textContent = results.map(result => `#${String(result.dex).padStart(4, "0")} ${result.status}`).join(", ");
    state.history.push({ at: new Date().toISOString(), list: state.activeList, count: results.length, sample: results.slice(0, 6) });
    state.history = state.history.slice(-30);
    addLog("info", "Scan applied", { mode, list: state.activeList, motion, count: results.length, sample: results.slice(0, 8) });
    saveState();
    renderAll();
    requestMissingNames();
    renderPreviewFrame();
    ...
    addLog("error", "Scan failed", { mode, list: state.activeList, error: error.message });
  } finally {
    isWorkerBusy = false;
...
async function scanGrid(sourceCanvas, rect) {
  ...
  for (const tile of anchorTiles) {
    const read = await readDexNumber(worker, sourceCanvas, tile.rect);
    if (!read?.dex) continue;
    anchorReads.push({ index: tile.index, dex: read.dex, confidence: read.confidence ?? 0 });
  }
  ...
  return tiles.map(tile => ({
    ...
    confirmed: anchorReads.some(read => read.index === tile.index && read.dex === baseDex + tile.index)
...
function chooseBaseDex(anchorReads, tileCount) {
  ...
  anchorReads.forEach(read => {
    ...
    if (candidate < 1 || candidate + tileCount - 1 > 9999) return;
    support.set(candidate, (support.get(candidate) || 0) + 1);
  });

  let bestCandidate = null;
  let bestCount = 0;
  for (const [candidate, count] of support.entries()) {
    if (count > bestCount) {
  ...
  for (let i = 0; i < sorted.length; i += 1) {
    for (let j = i + 1; j < sorted.length; j += 1) {
      if (sorted[j].dex - sorted[i].dex === sorted[j].index - sorted[i].index) {
...
function classifyTile(sourceCanvas, tileRect) {
  ...
  spriteCtx.drawImage(sourceCanvas, tileRect.x, tileRect.y, tileRect.width, spriteHeight, 0, 0, tileRect.width, spriteHeight);
  ...
  for (let i = 0; i < data.length; i += 4) {
    ...
    spreadTotal += spread;
    if (saturation > 0.12 && spread > 18) strongColorPixels += 1;
    if (saturation > 0.24 && spread > 28) vividPixels += 1;
  }

  const totalPixels = data.length / 4;
  const coverage = maskCount / totalPixels;
  const colorRatio = maskCount ? strongColorPixels / maskCount : 0;
  const vividRatio = maskCount ? vividPixels / maskCount : 0;
  const meanSaturation = maskCount ? saturationTotal / maskCount : 0;
  const meanSpread = maskCount ? spreadTotal / maskCount : 0;

  if (coverage < 0.028) return "missing";
  if (colorRatio > 0.24 && (vividRatio > 0.08 || meanSaturation > 0.16 || meanSpread > 26)) return "owned";
  return "seen";
}
...
function applyScanResults(results) {
  ...
  results.forEach(result => {
    ...
    if (statusRank(result.status) >= statusRank(current.status || "unknown")) {
      listEntries[result.dex] = { status: result.status, updatedAt: new Date().toISOString() };
    }
  });
}
...
function getPokemonName(dex) {
  return nameCache[dex] || `Dex #${String(dex).padStart(4, "0")}`;
}
...
function getMissingEntries() {
  ...
  LIST_TYPES.forEach(list => {
    Object.entries(state.lists[list.id]).forEach(([dex, entry]) => {
      if (entry.status === "missing") missing.push({ dex: Number(dex), list: list.id });
    });
  });
  return missing;
}
...
function renderMatches() {
  ...
  const rows = getMissingEntries()
    .flatMap(entry => opportunities
      .filter(opportunity => opportunity.list === entry.list && Number(opportunity.dex) === entry.dex)
      .map(opportunity => ({ ...entry, opportunity })))
...
function formatDateRange(start, end) {
  ...
  return endDate ? `${startText} - ${endDate.toLocaleString()}` : startText;
}
...
function requestMissingNames() {
  const dexNumbers = [...new Set(getMissingEntries().map(entry => entry.dex))]
    .filter(dex => !nameCache[dex])
    .slice(0, 12);
...
async function handleUpload(event) {
  ...
  addLog("info", "Image uploaded for scanning", { width: image.width, height: image.height, name: file.name });
