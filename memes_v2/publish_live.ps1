...
try {
  Write-Host "Creating tracked-files-only deploy bundle..."
  ...
  $legacyMemesPath = Join-Path $deployRoot "memes"
  if (Test-Path -LiteralPath $legacyMemesPath) {
    Remove-Item -LiteralPath $legacyMemesPath -Recurse -Force
  }
